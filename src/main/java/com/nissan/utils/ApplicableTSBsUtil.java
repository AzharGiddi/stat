...
public class ApplicableTSBsUtil {

	private ApplicableTSBsUtil() {
	...
	/*public static List<String> getApplicableTSBs(String modelCode, String modelYear, String componentCategory) {
		...
		return APIUtil.getResponse("D_ApplicableTSBs_Automation", params).jsonPath().getList("pxResults.pzInsKey");

	}*/

	/*public static List<pxResults> getApplicableTSB(String modelCode, String modelYear, String componentCategory) {
		String baseUri = ConfigurationManager.getBundle().getString("base.uri");
		String url = baseUri+String.format(
				"data/D_ApplicableTSBs_Automation?ModelCode=%s&ModelYear=%s&ComponentCategory=%s",
				modelCode, modelYear, componentCategory);
		String responseString = APIUtil.getResponseObject(url).asString();
		return new Gson().fromJson(responseString, ApplicableTSBListObject.class).pxResults;

	}*/
	
	public static List<pxResults> getApplicableTSB(String modelCode, String modelYear, String componentCategory, String typeOfRule) {
		String baseUri = ConfigurationManager.getBundle().getString("base.uri");
		String url = baseUri+String.format(
				"data/D_ApplicableTSBs_Automation?ModelCode=%s&ModelYear=%s&ComponentCategory=%s&TypeOfRule=%s",
	...
	public static JsonToJavaObjectUtil getTSBContentAsObject(String tSBId) {
		String baseUri = ConfigurationManager.getBundle().getString("base.uri");
		String url = baseUri+String.format("cases/%s", tSBId);
		String responseString = APIUtil.getResponseObject(url).asString(); 
	...
	public class ApplicableTSBListObject{
	...
	public void test() {
		...
		JsonToJavaObjectUtil map = getTSBContentAsObject("NSA-STAT-WORK-F1-TSB SB-30030");
